#!/usr/bin/env python3
"""Hermes pre_llm_call adapter for the shared Sage router."""
...
def main() -> int:
    ...
    prompt = extra.get("user_message", "") if isinstance(extra, Mapping) else ""
    if not prompt and isinstance(envelope.get("user_message"), str):
        prompt = envelope["user_message"]
