...
rules, workflow gates, engineering principles — 398 lines, ~4.4k tokens,
always in context) enforces what must never be skipped. The **lazy
layer** (capabilities like TDD discipline, coding principles, systematic
...
That 398 is measured, not estimated — see
[docs/context-budget.md](docs/context-budget.md), regenerated by
`context_budget.py` and held in place by a CI budget. This section used to
claim "~200 lines". It had been wrong by a factor of two for a long time,
because nothing counted it; context growth is the definitive slow leak.
